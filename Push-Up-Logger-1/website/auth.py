    ...
    if user:
        if user.role == 'client':
            if check_password_hash(user.password, password):
                ...
                print('profileeeeeeeeeeeeeeeeeeeeeeeeeee')
...
def signup_post():
    ...
    if user:
        ...
        new_user = User(email=email, fullname=name, password=generate_password_hash(password, method='sha256'),role = 'client')
...
def signup_admin_post():
    ...
    if user:
        ...
        new_user = User(email=email, fullname=name, password=generate_password_hash(password, method='sha256'),role = 'admin')
        db.session.add(new_user)
        db.session.commit()
        # login_user(new_user, remember=True)
        flash('Account created!', category='success')
        return redirect(url_for('view.adminprofile')) #auth  blueprint ka nam ha
...
def admin_signup_laundry_post():
    ...
    # return render_template("signup.html", user=current_user)   
    return redirect(url_for('auths.adminsignuplaundry'))  #agar email pehla ka ho ga to flash (email already exist)
